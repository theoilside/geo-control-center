    ...
    return (
        <VStack spacing={'30px'}>
            ...
            <Flex width={'100%'} direction={'column'} gap={'15px'}>
                    ...
                    >
                        ...
                        <Tbody>
                            {getFilteredObjects(isDeletedIncluded)
                                .slice(Math.max((currentPage - 1) * ROWS_PER_PAGE - 1, 0), (currentPage) * ROWS_PER_PAGE)
                                .map((country) => (
                                    <Tr width={'100%'}>
                                        <Td
                                            color={'orange.700'}
                ...
                <HStack>
                    <Button onClick={handlePrevPageClick} size={'sm'}>←</Button>
                    <Text>Страница {currentPage} из {getTotalPageCount(countries.length)}</Text>
                    <Button onClick={handleNextPageClick} size={'sm'}>→</Button>
                </HStack>
            </Flex>
        </VStack>
